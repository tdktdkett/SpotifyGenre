...
```
As we can see, the classification accuracy for the short model excluding the variable liveness is slightly better than that of the model including liveness. 

#Loop to optimize the number of terminal nodes used in the classifier for the four most important variables based on the intial classifier
```{r}
rf_acc<-list()
ntrees<-list()
rf_models<-list()
...
```{r}
data_frame(Accuracy = unlist(rf_acc)) %>%
  mutate(TerminalNodes = row_number()) %>%
  ggplot(aes(TerminalNodes, Accuracy)) + 
  geom_col(aes(fill = TerminalNodes == which.max(Accuracy)))+
...
best.acc<-rf_acc[best.tree]

paste("The number of terminal nodes with the highest classification accuracy rate is ", best.tree ," and the rate is ", best.acc, "%")
```

Loop to optimize the number of variables selected at each node split
```{r}
mtry_acc<-list()
mtrys<-list()
rf_models<-list()
...
for (i in 2:9) {
  mtrys[[i]]<-i
  rf_models[[i]]<-sd.rf<-randomForest(genre~. -liveness, data=train, mtry=i, nodesize=14, importance=TRUE)
  rf_pred<-predict(rf_models[[i]], newdata = test)
...
best.mtryacc<-rf_acc[best.mtry]

paste("The number of trees with the highest classification accuracy rate is ", best.mtry ," and the rate is ", best.mtryacc, "%")
```
...
#Loop to optimize the pruning of the decision tree to determine optimal number of terminal nodes
```{r}
dt_acc<-list()
size<-list()
dt_models<-list()
...
Based on the optimization loop, the decision tree with the highest classification accuracy rate has 41 terminal nodes for each tree and has a classification error accuracy rate of 27.76%
...
#Loop to optimize K-value based on classification accuracy
```{r}
set.seed(1)
